...
void Player::UpdateArea(uint32 newArea)
{
    ...
    if (area && area->IsSanctuary() || GetAreaId() == 3475 || GetAreaId() == 327 || GetAreaId() == 3638) // Replace the 100 with your area ID    // in sanctuary
    {
    ...
    for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
    {
        ...
        if (Creature* creature = source->ToCreature())
        {
            ...
            switch (itr->second.OptionType)
            {
                ...
                {
                    VendorItemData const* vendorItems = itr->second.ActionMenuId ? sObjectMgr->GetNpcVendorItemList(itr->second.ActionMenuId) : creature->GetVendorItems(this);
...
bool Player::BuyItemFromVendorSlot(ObjectGuid vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    VendorItemData const* vItems = currentVendor ? sObjectMgr->GetNpcVendorItemList(currentVendor) : creature->GetVendorItems(this);
